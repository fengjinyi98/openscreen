  ...
  const handleImageUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!validTypes.includes(file.type)) {
      toast.error(t('Invalid file type'), {
        description: t('Please upload a JPG, PNG, GIF, or WebP image file.'),
      });
      event.target.value = '';
      return;
    }
      ...
      <div className="mb-6">
        ...
        <Tabs value={annotation.type} onValueChange={(value) => onTypeChange(value as AnnotationType)} className="mb-6">
          ...
          <TabsContent value="text" className="mt-0 space-y-4">
            <div>
              <label className="text-xs font-medium text-slate-200 mb-2 block">{t('Text Content')}</label>
              <textarea
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="text-xs font-medium text-slate-200 mb-2 block">{t('Font Style')}</label>
                  <Select 
                ...
                <div>
                  <label className="text-xs font-medium text-slate-200 mb-2 block">{t('Size')}</label>
                  <Select 
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-xs font-medium text-slate-200 mb-2 block">{t('Text Color')}</label>
                  <Popover>
                ...
                <div>
                  <label className="text-xs font-medium text-slate-200 mb-2 block">{t('Background')}</label>
                  <Popover>
                    <PopoverTrigger asChild>
                      <Button 
                        variant="outline" 
                      ...
                      >
                        ...
                        <span className="text-xs text-slate-300 truncate flex-1 text-left">
                          {annotation.style.backgroundColor === 'transparent' ? t('None') : t('Color')}
                        </span>
                        <ChevronDown className="h-3 w-3 opacity-50" />
                      </Button>
                    </PopoverTrigger>
          ...
          <TabsContent value="image" className="mt-0 space-y-4">
            ...
            <p className="text-xs text-slate-500 text-center leading-relaxed">
              {t('Supported formats: JPG, PNG, GIF, WebP')}
            </p>
          </TabsContent>

          <TabsContent value="figure" className="mt-0 space-y-4">
            <div>
              <label className="text-xs font-medium text-slate-200 mb-3 block">{t('Arrow Direction')}</label>
            ...
            <div>
              <label className="text-xs font-medium text-slate-200 mb-2 block">
                {t('Stroke Width: {{width}}px', { width: annotation.figureData?.strokeWidth || 4 })}
              </label>
              <Slider
            ...
            <div>
              <label className="text-xs font-medium text-slate-200 mb-2 block">{t('Arrow Color')}</label>
              <Popover>
                <PopoverTrigger asChild>
                  <Button 
                    variant="outline" 
        ...
        <div className="mt-6 p-3 bg-white/5 rounded-lg border border-white/5">
          <div className="flex items-center gap-2 mb-2 text-slate-300">
            ...
            <span className="text-xs font-medium">{t('Shortcuts & Tips')}</span>
          </div>
          <ul className="text-[10px] text-slate-400 space-y-1.5 list-disc pl-3 leading-relaxed">
            <li>{t('Move playhead to overlapping annotation section and select an item.')}</li>
            <li>
              {t('Use')}{' '}
              <kbd className="px-1 py-0.5 bg-white/10 rounded text-slate-300 font-mono">Tab</kbd>{' '}
              {t('to cycle through overlapping items.')}
            </li>
            <li>
              {t('Use')}{' '}
              <kbd className="px-1 py-0.5 bg-white/10 rounded text-slate-300 font-mono">Shift+Tab</kbd>{' '}
              {t('to cycle backwards.')}
            </li>
          </ul>
        </div>
      </div>
